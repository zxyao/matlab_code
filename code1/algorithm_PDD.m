function [theta_t, theta_r] = algorithm_PDD(M,N,L,H_RB,H_mB,H_nB,h_mR,h_Rn,H_mn,H_nn,gamma_const,alpha,rho_opt,s_m,t_const1,t_const2,P_m,P_t)
...
theta_t = sqrt(0.5)*exp(1i * 2*pi*rand(L,1));
theta_r = sqrt(0.5)*exp(1i * 2*pi*rand(L,1));
...
for i = 1:200
    ...
    % optimize AL problem
    [W, theta_t, theta_r, theta_tt, theta_rr] = algorithm_BCD(M,N,L, theta_t, theta_r, theta_tt, theta_rr, lambda_t, lambda_r, rho,H_RB,H_mB,H_nB,h_mR,h_Rn,H_mn,H_nn,gamma_const,alpha,rho_opt,s_m,t_const1,t_const2,P_m,P_t);
