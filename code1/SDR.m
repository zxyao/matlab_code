function [vr,vt, lower_bound] = SDR(N,P_m,P_t,L,h_RB,H_mB,H_nB,h_mR,h_Rn,h_mn,h_nn,gamma_const,alpha,M,rho1,s_m1,t_const11,t_const22)
    W_sigma=dBm_W(-110);
    LL = 1000;
    ...
    for k=1:M
        h(:,:,k)=h_rb'.*h_mR(:,:,k);
        H(:,:,k)=P_m(k).*[h(:,:,k)*h(:,:,k)',h(:,:,k)*h_mb(k)'; h_mb(k)*h(:,:,k)',h_mb(k)'*h_mb(k)]; 
    end
     %% 
    ans_h=zeros(L+1,L+1);
    for k=1:M
    ...
    for i=1:N
        ...
        for j=1:M 
            h1(:,:,i)=h_Rn(:,:,i).*h_mR(:,:,j);
            W(:,:,i)=W(:,:,i)+alpha(j,i)*P_m(j).*[h1(:,:,i)*h1(:,:,i)',h1(:,:,i)*h_mn(j,i)'; h_mn(j,1)*h1(:,:,i)',h_mn(j,i)*h_mn(j,i)']; %¸¨Öú±äÁ¿W
        end
    end
    ...
    cvx_begin sdp quiet
       ...
       % % Calculated safe driving factor
       ...
       for i=1:M
        ...
        subject to
%             diag(V_r)<=1;
%             diag(V_t)=1*(1-diag(V_r));
%             diag(V_t)<=1;
%             for i=1:L
%                 V_t(i,i)==1.1*(1-V_r(i,i));
%             end
              diag(V_t)+diag(V_r)==1;
%             diag(V_r)>=0;
%             diag(V_t)>=0;
            ...
            V_t>=0;
            V_r(L+1,L+1)==1;
            sum_left<=sum_right*10^(-3);
    cvx_end
    lower_bound = cvx_optval;
    % Gaussian randomization process
    %% method 1
    max_F = 0;
    max_v = 0;
    sum_Sm=0;
